...
| `TOPAZ_MODEL` | `prob-3` | Model used by `tvai_up`. |
| `AUDIO_MODE` | `auto` | `auto` = stream copy when the codec is mp4-safe, otherwise re-encode to AAC. `copy` / `reencode` force one. |
...
| `409 JOB_ACTIVE` when deleting | Cancel the job first; deletion is refused while it is queued/rendering. |
